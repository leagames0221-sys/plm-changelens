"""PLM ChangeLens demo video — fully automated build pipeline.
...
Run:
  ...
  -> video/out/plm_changelens_demo.mp4  (~90s, 1080p)
...
def _scenes_factory() -> list[Scene]:
    """Build the per-scene scroll actions over the demo viewer (PLM ChangeLens).

    demo-viewer section anchors (docs/demo-viewer/index.html):
      .sec-hero / .sec-problem / .sec-input / .sec-diff / .sec-impact /
      .sec-drafts / .sec-trace / .sec-split / .sec-free / .sec-license

    Narration rules: keep commas to 1-2 per line, spell roman brands in kana
    (AI -> エーアイ, EARS -> イアース, MIT -> エムアイティー), and use plain words
    (BOM -> 部品表, traceability -> 対応表).
    """
    ...
    return [
        Scene("S1", 8.5, s1, "部品表の変更から、 変更影響と書類の下書きをエーアイが提案するツール、 チェンジレンズです。"),
        Scene("S2", 9.0, s2, "医療機器は部品を変えるたび、 要件や設計やテストの書類をすべて直す必要があります。"),
        Scene("S3", 8.0, s3, "まず新しいものと古いもの、 2つの部品表を入れます。 階層と数量と版を読み込みます。"),
        Scene("S4", 9.5, s4, "2つの差を決まった計算で正確に洗い出し、 追加や削除や数量変更や版更新に分けます。 幻覚は起きません。"),
        Scene("S5", 9.0, s5, "洗い出した差をエーアイが日本語で要約します。 各説明には根拠の部品がひも付きます。"),
        Scene("S6", 9.0, s6, "直すべき要件やテストの下書きをエーアイが提案します。 人は確認して直すだけです。"),
        Scene("S7", 8.5, s7, "対応表のどこが影響するか、 新しく必要な対応はどれかを示します。"),
        Scene("S8", 9.5, s8, "命にかかわる計算は決まった処理で確実に。 文章の下書きだけをエーアイが補助します。"),
        Scene("S9", 8.5, s9, "すべて無料でカード登録も不要。 エーアイはローカルでも動き、 外部への依存はゼロです。"),
...
def compose_final(webm: Path, narration: Path, out_mp4: Path) -> None:
    ...
    credit_path.write_text(
        "PLM ChangeLens Demo (PoC) / TTS: AivisSpeech (JP narrator) / synthetic data only",
        encoding="utf-8",
    )
    ...
    subprocess.run(
        [
            ...
            "-c:a", "aac", "-b:a", "128k",
            "-shortest",
            "-metadata", "comment=PLM ChangeLens Demo PoC / synthetic data only",
            str(out_mp4),
        ],
        check=True,
    )
...
def main() -> int:
    ...
    info("=== PLM ChangeLens demo video pipeline ===")
    if narration_only:
    ...
    out_mp4 = OUTPUT_DIR / "plm_changelens_demo.mp4"
